...
module.exports = class Article extends require('./model') {
    ...
    static getList() {
        return new Promise((resolve, reject) => {
            let sql = 'SELECT *  FROM article ORDER BY date DESC'
    ...
    static getListByCategoryId(id) {
        return new Promise((resolve, reject) => {
            let sql = 'SELECT * FROM article WHERE category_id = ? ORDER BY date DESC'
    ...
    static getListBykeywrod(keyword) {
        return new Promise((resolve, reject) => {
            let sql = 'SELECT *  FROM article WHERE title LIKE ? ORDER BY date DESC'
    ...
    static getArticleById(id) {
        return new Promise((resolve, reject) => {
            let sql = 'SELECT a.id,a.`thumbnail`,a.title,a.content,a.`date`,a.hits,a.`category_id`,c.`name`,a.`icon`,a.`hot` FROM article a,category c WHERE a.`category_id` = c.`id` AND a.id = ?'
            this.query(sql, id).then(results => {
                resolve(results[0])
            }).catch(err => {
    ...
    static getPage(start, size, category_id, hot) {
        return new Promise((resolve, reject) => {
            let sql = 'SELECT id,title,`icon`,hot FROM article WHERE 1=1'
